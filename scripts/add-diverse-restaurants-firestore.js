...
if (!userId) {
  console.error('❌ Please provide a user ID: node scripts/add-diverse-restaurants-firestore.js YOUR_USER_ID');
  process.exit(1);
}
...
async function addDiverseRestaurants() {
  try {
    console.log('🌍 Starting to add diverse restaurants to Firestore...');
    ...
    for (const restaurant of diverseRestaurants) {
      const cuisine = cuisineMap[restaurant.cuisineId] || 'Unknown';
    ...
    console.error('❌ Error adding diverse restaurants:', error.message);
  } finally {
    process.exit(0);
  }
}
